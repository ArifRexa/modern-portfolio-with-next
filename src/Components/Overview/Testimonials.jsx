  ...
  useEffect(() => {
    const fetchAllTestimonials = async () => {
      try {
        ...
        if (linkedinError) {
          console.error('Error fetching LinkedIn recommendations:', linkedinError);
        }
        ...
        // Combine the data with a type indicator
        const linkedinRecommendations = (linkedinData || []).map(rec => ({
          ...rec,
          type: 'linkedin',
          name: rec.recommender_name,
          role: rec.title ? `${rec.title}${rec.worked_together_on ? ` at ${rec.worked_together_on}` : ''}` : 'LinkedIn Connection',
          avatar: rec.profile_image_url,
          content: rec.recommendation_text,
          company: rec.worked_together_on || '',
          designation: rec.title || '',
          location: '',
        ...
        const testimonials = (testimonialData || []).map(t => ({
          ...t,
          type: 'testimonial',
          role: t.designation ? `${t.designation} at ${t.company}` : t.company,
          avatar: t.avatar_url,
          content: t.testimonial
        }));

        // Combine both arrays with LinkedIn recommendations first
        const combinedData = [...linkedinRecommendations, ...testimonials];
        setAllTestimonials(combinedData);
      } catch (err) {
    ...
    return (
      <div className={`${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'} backdrop-blur-md rounded-xl border ${theme === 'dark' ? 'border-gray-700/50' : 'border-gray-300'} p-4 lg:p-6 shadow-sm`}>
        ...
        <div className="flex justify-center items-center py-12">
          ...
          <span className={`ml-2 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Loading testimonials...</span>
        </div>
      </div>
    );
  }
      ...
      <div className={`${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'} backdrop-blur-md rounded-xl border ${theme === 'dark' ? 'border-gray-700/50' : 'border-gray-300'} p-4 lg:p-6 shadow-sm`}>
        ...
        <div className="flex justify-center items-center py-12">
          <div className="text-center">
            <p className={`text-red-400 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Error loading testimonials: {error}</p>
            <p className={`text-sm mt-2 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Please try again later.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {currentTestimonials.map((testimonial, idx) => (
                  <div key={startIndex + idx}>
                    {testimonial ? (
                      <div
                        className={`bg-gradient-to-br ${theme === 'dark' ? 'from-gray-800 to-gray-900 border-gray-700/40' : 'from-gray-200 to-gray-50 border-gray-300'} rounded-xl p-4 lg:p-4 shadow-md backdrop-blur-md min-h-56 h-full cursor-pointer transition-transform hover:scale-[1.01] hover:shadow-md duration-300 ease-in-out`}
      ...
      {showModal && selectedTestimonial && (
        ...
        >
          <div
            className={`bg-gradient-to-br ${theme === 'dark' ? 'from-gray-800 to-gray-900' : 'from-gray-200 to-gray-50'} rounded-xl border ${theme === 'dark' ? 'border-gray-700/50' : 'border-gray-300'} w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 relative shadow-xl`}
          ...
          >
            ...
            <div className="flex items-start space-x-4">
              <Image
                src={selectedTestimonial.avatar || avatarPlaceholder}
              ...
              />
              <div>
                <h3 className={`text-xl font-bold ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>{selectedTestimonial.name}</h3>
            ...
            <div className="mt-6 flex justify-end">
              <button
                onClick={() => setShowModal(false)}
                className={`px-4 py-2 rounded-lg transition-colors ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600 text-gray-200' : 'bg-gray-300 hover:bg-gray-400 text-gray-700'}`}
              >
                Close
              </button>
            </div>
