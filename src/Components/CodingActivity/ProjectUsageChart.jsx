...
const ProjectUsageChart = ({ projectsData, chartType = 'bar', theme }) => {
  if (!projectsData || projectsData.length === 0) {
    return (
      <div className={`${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'} backdrop-blur-md rounded-xl border ${theme === 'dark' ? 'border-gray-700/50' : 'border-gray-300'} p-6 shadow-sm h-80 flex items-center justify-center`}>
        <p className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>No project data available</p>
      </div>
    );
  }
  ...
  const chartData = projectsData.map(project => ({
    name: project.name,
    hours: parseFloat(project.decimal),
    time: project.text,
    percent: project.percent,
    humanAdditions: project.human_additions || 0,
    humanDeletions: project.human_deletions || 0
  }));
  ...
  return (
    <div className={`${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'} backdrop-blur-md rounded-xl border ${theme === 'dark' ? 'border-gray-700/50' : 'border-gray-300'} p-4 lg:p-6 shadow-sm h-80`}>
      <h3 className={`text-xl font-bold mb-4 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>Project Activity</h3>
