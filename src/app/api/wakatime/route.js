...
export async function GET(request) {
  try {
    ...
    // Try to fetch fresh data from WakaTime API
    if (apiKey) {
      try {
        ...
        if (!response.ok) {
          ...
          console.warn(`WakaTime API error ${response.status}: ${errorText}`);
        } else {
          const wakaData = await response.json();

          // --- Prepare Supabase upsert data ---
          const daysOfWeek = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
          recordsToUpsert = wakaData.data.map(day => {
          ...
          weeklyActivity = recordsToUpsert.map(record => ({
        ...
        console.warn("Error fetching from WakaTime API, falling back to Supabase data:", wakaError.message);
      }
    } else {
      console.warn("WAKA_TIME_API_KEY not configured, using only Supabase data");
    }

    // If we couldn't get fresh data from WakaTime, fetch from Supabase instead
    if (recordsToUpsert.length === 0) {
      ...
      if (supabaseError) {
        ...
        return NextResponse.json({ error: "Failed to fetch data from Supabase", message: supabaseError.message }, { status: 500 });
      }

      const daysOfWeek = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
      weeklyActivity = supabaseData.map(record => ({
        day: daysOfWeek[dayjs(record.date).day()],
        hours: record.coding_time.toFixed(2),
        formatted: formatHoursToTime(record.coding_time),
      }));
    ...
    // Use the most recent data for today's coding time
    const mostRecentRecord = recordsToUpsert.length > 0
      ? recordsToUpsert[recordsToUpsert.length - 1]
      : (weeklyActivity.length > 0
        ? { coding_time: parseFloat(weeklyActivity[weeklyActivity.length - 1].hours) }
        : { coding_time: 0 });
    ...
    return NextResponse.json({
      ...
      extendedData: {
        coding_time: {
          today: formatHoursToTime(mostRecentRecord.coding_time),
          weekly: formatHoursToTime(weeklyActivity.reduce((sum, day) => sum + parseFloat(day.hours), 0)),
          monthly: formatHoursToTime(monthlyTotal),
          average: formatHoursToTime(avgDaily),
          today_numeric: mostRecentRecord.coding_time,
        },
    ...
    return NextResponse.json({ error: "Failed to fetch or update data", message: err.message }, { status: 500 });
  }
}
